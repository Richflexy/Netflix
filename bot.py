...
# ================= CONFIG =================
TOKEN = "8730805668:AAGnUrxv2NGZ0X1tcBoPVOLeTHzOq2PT-mQ"  # Your Netflix bot
OWNER_ID = 8260250818
PROOF_CHANNEL_ID = -1003811293838  # Replace with your channel ID
...
waiting_proof = set()  # users waiting to send screenshot
...
f"❤️ +1 Referral Done\n👤 New user joined using your link!\n💰 You earned 1 point {kind_emoji()}")
        ...
        if chat_id==OWNER_ID: pts="Unlimited 🔥"
...
🎊Thanks For Using Our Bot🎊~Send Screenshot To your proof channel ❣️""")
            waiting_proof.add(chat_id)
            return

        if users.get(chat_id,{}).get("points",0) >= 2:  # 2 points per redeem
            if chat_id not in redeemed_urls: redeemed_urls[chat_id] = []
...
def receive_proof(msg):
    chat_id = msg.chat.id
    if chat_id not in waiting_proof: return

    username = msg.from_user.username
    name = msg.from_user.first_name
    caption = f"📸 Netflix Login Proof\n👤 Name: {name}\n🔗 Username: @{username}\n🆔 User ID: {chat_id}"

    bot.forward_message(PROOF_CHANNEL_ID, chat_id, msg.message_id)
    bot.send_message(PROOF_CHANNEL_ID, caption)
