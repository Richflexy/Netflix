...
# ================= CONFIG =================
TOKEN = "8730805668:AAGnUrxv2NGZ0X1tcBoPVOLeTHzOq2PT-mQ"  # Netflix bot
PROOF_BOT_ID = 3811293838  # Your proof bot/channel chat ID
OWNER_ID = 8260250818
...
def save_redeemed():
    with open(REDEEM_FILE, "w") as f:
        json.dump(redeemed_urls, f)

def load_redeemed():
    global redeemed_urls
    if os.path.exists(REDEEM_FILE):
        with open(REDEEM_FILE, "r") as f:
            redeemed_urls = json.load(f)
    else:
        redeemed_urls = {}
...
🆔 User ID: {chat_id}
"""
    bot.forward_message(PROOF_BOT_ID, chat_id, msg.message_id)
    bot.send_message(PROOF_BOT_ID, caption)
...
Total Points Given: {total_points}
Total Referrals: {total_invited}""")
